    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          ...
          {
            params: { subcategory: "Tissue Box Covers" },
          }
        );
        setProducts(response.data.data.docs); // Assuming paginated response
      } catch (error) {
        console.error("Error fetching Tissue Box Covers products:", error);
      } finally {
...
export default TissueBoxCovers;
